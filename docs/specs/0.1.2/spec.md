# vTON DAO Governance Model — 0.1.2
...
### 2.5 Proportional Burn

Proposer는 제안 생성 시 선택적으로 burn rate를 설정할 수 있다. Burn rate가 설정된 제안에 투표하면, 투표에 사용된 vTON의 해당 비율이 소각된다. 이 메커니즘은 중요한 제안에 대해 투표자가 더 신중하게 의사결정에 참여하도록 유도한다.

| 파라미터 | 범위 | 설명 |
|---------|------|------|
| Burn Rate | 0% ~ 100% | Proposer가 제안 생성 시 설정하는 투표 소각 비율 |
...
Security Council 구성원은 별도의 임기 제한이 없으며, DAO 투표를 통해 해임되기 전까지 직위를 유지한다.
