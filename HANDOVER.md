...
5. **테스트 현황**: 2026-07-06 기준 `forge test` 354개 전체 통과 (핵심 컨트랙트 라인 커버리지 94~98%). 스펙 0.1.4 이전에 작성되어 자기방어 제한과 모순되던 가디언 취소 테스트 2개를 스펙에 맞게 수정했습니다.
6. **2026-07-06 리팩토링 진단에서 확인된 미해결 이슈** (메인넷 진행 전 반드시 검토):
   - **VoteRelayFund 자금 유출 벡터 (높음)**: `relayVote`의 `governor` 파라미터가 임의 주소를 허용하고 가스 환급이 `tx.gasprice` 무제한 — 악의적 릴레이어가 agent 예치금을 빼갈 수 있음. governor 고정 + 가스가 상한 + 릴레이어 허용목록 필요
   - **`registerDelegateFor` 그리핑 (중간)**: 누구나 임의 주소를 임의 프로필로 위임자 등록 가능. 대상 주소의 서명 요구 필요
   - **신규 위임자의 과거 소각 손실 분담 (중간)**: `burnFromDelegate` 발생 후 새로 위임한 사람도 회수 시 비례 손실을 짐 — 소각 회계가 명목/실질 단위를 혼용
   - **결정 보류 항목**: DelegateRegistry 체크포인트 기계(`_votingPowerCheckpoints`)는 온체인 소비처가 없는데 위임 연산마다 가스를 소모 — 오프체인 조회용으로 유지할지 제거할지 결정 필요. vTON.mint 곱셈 순서(정밀도), `queue()`의 pause 게이트 부재도 검토 대상
